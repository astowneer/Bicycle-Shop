...
{
    public class MountainBicycleController : Controller
    {
        //private readonly RepositoryContext<Bicycle> _bicycleContext;
        private readonly IServiceManager<Bicycle> _mainService;
        private readonly IServiceManager<Mountain> _service;

        public MountainBicycleController(IServiceManager<Bicycle> mainService,
            IServiceManager<Mountain> service)
        {
        ...
        public IActionResult Index()
        {
            ...
            var mountainBicycles = mainBicycles.Select(m => new MountainDto(m)).ToList();
            var mountainViewModel = CreateMountainViewModel(mountainBicycles, imagesPath);
            ...
            List<BrakeFilter> brakeFilter)
        {
            ...
            var mainBicycles = bicycles.Select(m => new Bicycle(m));
            var mountainBicycle = mainBicycles.Select(m => new MountainDto(m)).ToList();
            var mountainViewModel = CreateMountainViewModel(mountainBicycle, imagesPath);

            return View(mountainViewModel);
        }

        private MountainViewModel CreateMountainViewModel(List<MountainDto> bicycles, List<string> imagePath)
        {
            return new MountainViewModel()
            {
                Bicycles = bicycles,
        ...
        public IActionResult Create(Mountain bicycle)
        {
            // Define unique Id
            if (bicycle is null)
                throw new Exception();
            ...
            {
                Bicycle mainBicycle = new Bicycle(bicycle, numberOfGears: bicycle.NumberOfGears);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Edit(int? id, Mountain bicycle)
        {
            if (ModelState.IsValid)
            { 
                Bicycle mainBicycle = new Bicycle(bicycle, numberOfGears: bicycle.NumberOfGears);
        ...
        private static List<string> GetImagePath(int amount)
        {
            ...
            {
                int index = (i + 1) % 7;
                imagesPath.Add($"~/Images/mountain{index}.png");
            }
